...
If the user’s answers fail validation:
  ...
  * show an [Error summary component](./components/error-summary.md) at the top of the page, and move keyboard focus to it
  * show [Error message components](./components/error-message.md) next to fields with errors

Read guidance on [writing good error messages](./components/error-message/#be-clear-and-concise.md).
...
Only add this sort of validation if your user research shows that, on balance, it solves more problems for users than it causes. For example, the [Character count component](./components/character-count.md) shows users an error message when they go over the character limit. Because it’s important that users do not spend time and effort writing out a response that turns out to be too long.
...
If you spot a problem with this guidance you can [propose a change on GitHub](https://github.com/alphagov/govuk-design-system/edit/main/src/patterns/validation/index.md). 
