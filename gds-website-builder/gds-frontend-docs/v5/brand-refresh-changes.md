#  [](./v5/brand-refresh-changes/#updating-your-service-to-use-the-new-brand.md)Updating your service to use the new brand
In June 2025, GOV.UK started to refresh the brand across its products and services.
The changes affect these components:
  * [GOV.UK header](https://design-system.service.gov.uk/components/header/)
  * [GOV.UK footer](https://design-system.service.gov.uk/components/footer/)
  * [Cookie banner](https://design-system.service.gov.uk/components/cookie-banner/)
  * [Service navigation](https://design-system.service.gov.uk/components/service-navigation/)

##  [](./v5/brand-refresh-changes/#if-you-re-using-gov-uk-frontend-v5.md)If you’re using GOV.UK Frontend v5
Update to the latest v5 release of GOV.UK Frontend. Follow the instructions in the [release notes for GOV.UK Frontend v5.10.0](https://github.com/alphagov/govuk-frontend/releases/tag/v5.10.0) to enable the new brand.
##  [](./v5/brand-refresh-changes/#if-you-re-using-gov-uk-frontend-v4.md)If you’re using GOV.UK Frontend v4
Update to the latest v4 release of GOV.UK Frontend. Follow the instructions in the [release notes for GOV.UK Frontend v4.10.0](https://github.com/alphagov/govuk-frontend/releases/tag/v4.10.0) to enable the new brand.
##  [](./v5/brand-refresh-changes/#if-you-re-using-gov-uk-frontend-v1-v2-or-v3.md)If you’re using GOV.UK Frontend v1, v2 or v3
If you’re either:
  * still migrating from GOV.UK Template, GOV.UK Frontend Toolkit or GOV.UK Elements
  * required to continue to fully support Internet Explorer 11 or other earlier browsers

Update to the latest v4 release of GOV.UK Frontend. Follow the instructions in the [release notes for GOV.UK Frontend v4.10.0](https://github.com/alphagov/govuk-frontend/releases/tag/v4.10.0) to enable the new brand.
Otherwise, update to the latest v5 release of GOV.UK Frontend, and then follow the instructions in the [release notes for GOV.UK Frontend v5.10.0](https://github.com/alphagov/govuk-frontend/releases/tag/v5.10.0) to enable the new brand.
Follow the instructions in all the release notes from the version you are currently using, up to and including the version you are updating to. Pay particular attention to the release notes for any major releases.
For help understanding which version you’re currently using, and for more guidance on updating between major versions, see [staying up to date with changes](./v5/staying-up-to-date/#how-the-version-numbering-works.md).
##  [](./v5/brand-refresh-changes/#if-you-re-not-using-gov-uk-frontend.md)If you’re not using GOV.UK Frontend
If you’ve not yet started migrating from GOV.UK Template, GOV.UK Frontend Toolkit and GOV.UK Elements, see our [guidance on migrating from our old frameworks](./v4/migrating-from-legacy-products.md).
  * [View source](https://github.com/alphagov/govuk-frontend-docs/blob/master/source/v5/brand-refresh-changes/index.html.md.erb)
  * [Report problem](https://github.com/alphagov/govuk-frontend-docs/issues/new?body=Problem+with+%27Updating+your+service+to+use+the+new+brand%27+%28https%3A%2F%2Ffrontend.design-system.service.gov.uk%2Fv5%2Fbrand-refresh-changes%2F%29&labels=bug&title=Re%3A+%27Updating+your+service+to+use+the+new+brand%27)
  * [GitHub Repo](https://github.com/alphagov/govuk-frontend-docs)

  * [Accessibility](https://design-system.service.gov.uk/accessibility/)
  * [GOV.UK Design System](https://design-system.service.gov.uk/)
  * [GOV.UK Prototype Kit](https://govuk-prototype-kit.herokuapp.com/)

All content is available under the [Open Government Licence v3.0](https://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/), except where otherwise stated 
[© Crown copyright](https://www.nationalarchives.gov.uk/information-management/re-using-public-sector-information/uk-government-licensing-framework/crown-copyright/)
